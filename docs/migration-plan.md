...
> **Status:** M0–M3 complete — Postgres (Compose + schemas/roles + `raw.source_fetch`),
> ingest → raw → dbt → FastAPI all running on Postgres. Remaining: Dagster
> orchestration (M1 proper), daily schedule (M5), Metabase/Elementary (M2+), k3s
> (M7). The DuckDB slice and its specs were removed.
